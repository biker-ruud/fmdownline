...
public final class HttpsSession {
    ...
    public InputStream connect(URL url) throws IOException {
        HttpURLConnection connection = getConnection(stripUrlToPathAndQuery(url));
        if (connection == null) {
            return null;
        }
    ...
    public InputStream post(URL url, Map<String, String> postParamMap) throws IOException {
        if (state != SessionState.CONNECTED) {
            LOGGER.info("NOT CONNECTED! with host " + host);
            return null;
        }
        HttpURLConnection connection = getConnection(url.getPath(), postParamMap);
        if (connection == null) {
            return null;
        }
        InputStream inputStream = connection.getInputStream();
    ...
    public void disconnect() {
        try {
            URL url = new URL(UNSECURE_PROTOCOL + host);
            ...
            LOGGER.info("DISCONNECTED! with host " + host);
            this.host = null;
        }
    }
    ...
    private HttpURLConnection getConnection(String path, Map<String, String> postParamMap) throws IOException {
        ...
        if (connection.getResponseCode() >= HttpURLConnection.HTTP_MULT_CHOICE && connection.getResponseCode() < HttpURLConnection.HTTP_BAD_REQUEST) {
            ...
            if (relocatedTo != null) {
                if (relocatedTo.startsWith(UNSECURE_PROTOCOL + host)) {
                    LOGGER.info("Absolute redirection");
                    URL relocateUrl = new URL(relocatedTo);
                    this.requestUrl = relocateUrl;
                    LOGGER.info("Relocated!");
                    HttpURLConnection relocatedConnection = getConnection(stripUrlToPathAndQuery(relocateUrl));
